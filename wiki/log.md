...
## [2026-08-28] fix | 세 번째 폴백 대상 유형 발견 — 크롤링·요약은 "성공"했지만 본인 정보를 못 찾은 37명

사용자가 대시보드에서 전상민(20607, 화학공학과) 교수의 "AI 요약(홈페이지 기반)" 박스에
"홈페이지 원문에서 본인 관련 정보를 명확히 찾지 못했습니다"라는 문구가 그대로 노출되는 것을
발견해 제보. 확인해보니 `summarize_homepages.py`가 크롤링 원문에서 본인 정보를 못 찾으면
정확히 이 문장만 답하도록 지시하는데(가드레일 규칙 3), `get_homepage_summary()`는 이 경우도
"요약이 있다"고 취급해 폴백 대상에서 제외하고 있었음 — 총 **37명**이 이 상태였음(기존 42명과
합쳐 총 79명).

- `build_wiki.py`: 이 고정 문구를 `HOMEPAGE_SUMMARY_NOT_FOUND` 상수로 정의하고,
  `get_homepage_summary()`가 이 문구와 일치하면 빈 문자열로 취급하도록 수정 — 교원 페이지·
  대시보드 양쪽에서 이 무의미한 "요약"이 더 이상 안 뜨고, `summarize_faculty_fallback.py`의
  `needs_fallback()`이 자동으로 이 37명을 폴백 대상으로 잡아냄.
- `summarize_homepages.py`가 `build_wiki.HOMEPAGE_SUMMARY_NOT_FOUND`를 import해 같은 문구를
  참조하도록 수정 — 두 스크립트가 문구를 각자 하드코딩해 나중에 어긋나는 것을 방지.
- 새로 대상이 된 37명도 이전과 동일한 방식(이 세션이 위키 필드를 직접 읽고 수동 작성, 실적
  개수 언급 없이)으로 요약을 작성해 `faculty_fallback_summary.json`에 추가.
- 재빌드 후 확인: 이 고정 문구가 위키 어디에도 안 남아있음, idempotent, 폴백 79명 + 홈페이지
  요약 219명 = 298명(중복·누락 없음), 전상민 페이지·대시보드 모두 정상적인 폴백 요약으로
  교체된 것을 Playwright로 확인.

`wiki/open-questions.md`의 "42건" 표기를 "79건"으로 갱신.
